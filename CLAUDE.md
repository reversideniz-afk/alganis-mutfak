...
988 tarif (869 Türk + 28 İtalyan + 13 Uzak Doğu + 13 Orta Doğu + 13 Meksika + 13 Hint + 13 Balkan + 13 Akdeniz + 13 Fransız), 259 malzeme, 11 tarif kategorisi, 9 mutfak, 8 öğün grubu (Bugün ekranı). Faz 2'nin ilk 8-mutfaklık turu Fransız'la tamamlandı, Faz 3 (akıllı öneri ve defter) sürüm 3.8.0'da tamamlandı, Faz 4 aynı gün kısmen tamamlandı (sürüm 3.9.0) — sıradaki adım için YOL-HARITASI.md'ye bak. Alt menüde artık 6. sekme "Defterim" var: koleksiyonlar, pişirme geçmişi, "bu hafta ne pişirdim". Tarif panelinin altında puan (5 yıldız) + kişisel not var; pişirme modunun son adımındaki "Pişirdim ✓" düğmesi geçmişi besliyor. "Bugün ne pişirsem?" sıralaması artık favori/puan/geçmiş/saate göre ince ayar yapıyor (`js/oneri.js`) — malzeme tamlığı sıralamasını hiç ezmiyor, mevsim bilerek yok (veri yok). Tarif panelindeki besin bilgisi artık düz rakam değil, kalori + karbonhidrat/protein/yağ oranını gösteren bir çubuk grafik (`js/arayuz.js`: `besinGrafik`) — Faz 4'ün "adım içi zamanlayıcı" maddesi kullanıcı kararıyla bununla değiştirildi. Tarif araması (`AM.tarifAra`, `js/eslestir.js`) artık alan ağırlıklı (ad > kategori/mutfak > malzeme) puanlıyor ve tek harf yazım hatasını tolere ediyor. İlk açılışta üç adımlık, atlanabilir bir tanıtım var (`js/tanitim.js`). Görsel yok — tüm kartlar SVG ikonla çalışıyor (bkz. "Görsel üretim hattı — DURDURULDU"). Ayarlarda 6 renk paleti + yüksek kontrast, tema (gün ışığı/gece/sistem), yazı boyutu, ölçü cetveli var. Malzeme/favori seçimleri katalogla karşılaştırılmadan saklanır (veri kaybını önlemek için — bkz. `js/depo.js` başındaki not). Güncel sürüm: `data/surum.js` → `AM.SURUM`.
